...
df = pd.read_csv('DETECTION_RESULTS.tsv', sep='\t')
...
def count_unique_by_condition(df_str, condition):
	filtered_df = df[df['positive_for_HPAI'] == condition]
	unique_counts = filtered_df.groupby('processing_plant_state')['carton'].nunique().reset_index()
...
results_df = pd.merge(false_counts, true_counts, on='processing_plant_state', how='outer').fillna(0)

## Rename the columns to match the required format
results_df.columns = ['Processing Plant State', 'Negative Cartons', 'Positive Cartons']

## Calculate column "Total Cartons" as the sum of columns "Negative Cartons" and "Positive Cartons"
results_df['Total Cartons'] = results_df['Negative Cartons'] + results_df['Positive Cartons']

## Find the latest date for each unique string in column '2'
latest_dates = df.groupby('processing_plant_state')['date_purchased'].max().reset_index()
latest_dates.columns = ['Processing Plant State', 'As of']

## Merge the latest dates into the "Results DataFrame"
results_df = pd.merge(results_df, latest_dates, on='Processing Plant State', how='left')

## Rearrange columns
sorted_results_df = results_df[['Processing Plant State', 'Total Cartons', 'Negative Cartons', 'Positive Cartons', 'As of' ]]
...
sorted_results_df.to_csv('assets/positivity_tally.tsv', sep='\t')
